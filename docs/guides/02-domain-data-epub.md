...
| **`library_books`** | Каталог книг: путь к EPUB, метаданные, размер, обложка, дата добавления, `total_chapters`, `is_favorite`, `deleted_at` (soft-delete / корзина). PK — `book_id`. |
...
- Индекс по `deleted_at` — выборки активных записей и корзины.
...
| `totalChapters` | Число пунктов spine; 0, пока книгу не открывали в читалке. |
| `isFavorite` | Флаг избранного. |
| `deletedAt` | Время помещения в корзину (`null` — книга в основном списке). |

Тип **`LibraryBookWithProgress`** — запись библиотеки плюс `lastChapterIndex` и `progressFraction` для списков (см. `StorageService`).
